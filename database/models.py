...
class Job(Base, models.ModelBase):
    ...
    worker_id = Column(Integer, ForeignKey('workers.id'), nullable=True)  # References Worker.id
    status = Column(String(50), nullable=False, default="submitted")  # e.g., submitted, running, completed
    submitted_date = Column(DateTime, default=datetime.utcnow, nullable=False)
    last_update_date = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow, nullable=False)
